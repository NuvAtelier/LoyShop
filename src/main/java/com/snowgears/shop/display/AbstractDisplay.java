...
public abstract class AbstractDisplay {
    ...
    /**
     * Updates the display tag for a specific player
     * @param player The player to update the display tag for
     */
    public void updateDisplayTag(Player player) {
        if (player == null || !player.isOnline()) {
            return;
        }

        // Remove existing display tags and show new ones
        removeDisplayEntities(player, true);
        showDisplayTags(player);
    }

    /**
     * Checks if this display needs to be updated
     * @return true if the display needs updating
     */
    public boolean needsUpdate() {
        // Default implementation - can be overridden by specific display types
        return false;
    }

    /**
     * Processes any pending updates for this display
     */
    public void processUpdate() {
        // Default implementation - can be overridden by specific display types
        // This method would handle any pending display updates
    }

    /**
     * Checks if a player is in the same world as this display
     * @param player The player to check
     * @return true if the player is in the same world as the display
     */
    protected boolean isSameWorld(Player player) {
        if (player == null || player.getWorld() == null || shopSignLocation.getWorld() == null) {
            return false;
        }
        return player.getWorld().getUID().equals(shopSignLocation.getWorld().getUID());
    }
